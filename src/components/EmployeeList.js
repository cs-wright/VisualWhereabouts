...
function EmployeeCard({ imgPathContactable, firstName, surname }) {
  const imageURL =
    "https://whowhatwhere.azurewebsites.net/" + imgPathContactable;
  return (
    <div className="card">
      <img
...
export default function EmployeeList({ selectedSubGroup, selectedButton }) {
  ...
  React.useEffect(() => {
    if (selectedSubGroup[0] == null) {
      api.get("/api/persons").then((response) => {
      ...
      axios.all(results).then((response) => {
        ...
        let peopleArray = idList.map((x) =>
          api.get("/api/persons/" + x + "/full")
        );
  ...
  peopleArray = person.map((element) => {
    return (
      ...
      >
        <EmployeeCard
          imgPathContactable={element.imgPathContactable}
